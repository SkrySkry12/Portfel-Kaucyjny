...
export function initDatabase(): void {
  ...
  d.execSync(`CREATE TABLE IF NOT EXISTS coupons (id TEXT PRIMARY KEY, storeId TEXT NOT NULL, amount REAL, barcode TEXT NOT NULL, couponNumber TEXT, status TEXT NOT NULL DEFAULT 'active' CHECK(status IN ('active','used')), originalOcrData TEXT, modifiedData TEXT, changeHistory TEXT NOT NULL DEFAULT '[]', dateAdded TEXT NOT NULL DEFAULT (datetime('now')), expiresAt TEXT, usedAt TEXT, FOREIGN KEY (storeId) REFERENCES stores(id) ON DELETE CASCADE)`);
  // Migration: add expiresAt column if missing (existing installs)
  try { d.execSync(`ALTER TABLE coupons ADD COLUMN expiresAt TEXT`); } catch {}
...
export function insertCoupon(data: { storeId: string; amount: number | null; barcode: string; couponNumber: string | null; originalOcrData?: string | null }): string {
  const id = randomUUID();
  getDb().runSync('INSERT INTO coupons (id, storeId, amount, barcode, couponNumber, originalOcrData, expiresAt) VALUES (?, ?, ?, ?, ?, ?, datetime(\'now\', \'+30 days\'))',
...
export function getExpiringCoupons(withinDays: number): CouponWithStore[] {
  return getDb().getAllSync<CouponWithStore>(
    `SELECT c.*, s.name as storeName FROM coupons c JOIN stores s ON c.storeId = s.id WHERE c.status = 'active' AND c.expiresAt IS NOT NULL AND c.expiresAt <= datetime('now', '+' || ? || ' days') AND c.expiresAt > datetime('now') ORDER BY c.expiresAt ASC`,
    [withinDays]
  ) ?? [];
}
...
export function getExpiredCoupons(): CouponWithStore[] {
  return getDb().getAllSync<CouponWithStore>(
    `SELECT c.*, s.name as storeName FROM coupons c JOIN stores s ON c.storeId = s.id WHERE c.status = 'active' AND c.expiresAt IS NOT NULL AND c.expiresAt <= datetime('now') ORDER BY c.expiresAt ASC`
  ) ?? [];
}
