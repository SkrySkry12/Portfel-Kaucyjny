...
export function insertCoupon(data: { storeId: string; amount: number | null; barcode: string; couponNumber: string | null; originalOcrData?: string | null }): string {
  const id = randomUUID();
  memCoupons.push({ id, storeId: data.storeId, amount: data.amount, barcode: data.barcode, couponNumber: data.couponNumber, status: 'active', originalOcrData: data.originalOcrData ?? null, modifiedData: null, changeHistory: '[]', dateAdded: new Date().toISOString(), usedAt: null });
  return id;
}
